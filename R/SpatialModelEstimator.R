...
#' 
#' @details 
#' Return Estimates and objects for GMRF approach
#'
...
SpatialModelEstimator = function(spatial_df, formula, mesh, extrapolation_grid, family, link, bias_correct = F, control = list(eval.max = 10000, iter.max = 10000), convergence = list(grad_tol = 0.001)) {
  ...
  return_list = list(est_init = est_init, sd_init = sd_init, est_ln_init = est_ln_init, sd_ln_init = sd_ln_init, init_convergence = init_convergence, MLE_init_report = MLE_init_report)
